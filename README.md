...
<table style="width: 100%;">
  ...
  <tbody>
    ...
    <tr>
      <td align="center" style="font-size: 12px;">WebSocket 기반 메시징 및<br>트랜잭션 구현, 성능테스트 및 최적화 수행</td>
...
#### 3. 소셜 로그인 시 JWT 토큰 방식 대신 세션 방식 채택  
...
## 6. 성능 테스트 및 최적화  
1. **실제 서비스 상황에서의 페이지네이션 방식의 성능 차이 및 인덱스 유무에 따른 메모리 사용량 측정**  
- 실제 상황(서울대 구성원 2만명, 각 인원 페이지네이션 횟수 최대 50회)을 가정하여 커서 기반 / 오프셋 기반 페이지네이션 각각의 페이지네이션 API의 응답 시간 및 인덱스 유무에 따른 메모리 사용량을 측정하여, 약 4MB의 추가 메모리 사용을 감수하고 데이터 정합성(메시지 누락/중복 출력 이슈)을 더욱 우선시하여 사용자 경험을 개선하였습니다.  
  
| 페이지네이션 방식 | avg | p(90) | p(95) | 메모리 사용량(MB) |   
| -------------- | --- | ----- | ----- | --------------- |  
| 오프셋 | 44.49ms | 62.23ms | 72.24ms | 30.0 |  
| 커서 | 39.54ms | 58.22ms | 70.62ms | 35.1 |  
  
2. **커서 기반 페이지네이션 시 인터페이스 교체 및 레포지토리 함수 통합으로 쿼리 횟수 감소**  
- 기존 Page 인터페이스의 불필요 정보(총 페이지 수)로 인한 추가 쿼리를 방지하기 위해, Slice 인터페이스로 개선 및 페이지네이션 로직의 함수 호출을 통합하여 쿼리 횟수를 5 -> 3회로 감소, p95 및 RPS 약 30% 개선(목표 RPS 1000 도달)  
  
| 개선 전/후 | avg | p(90) | p(95) | RPS(req / s) |  
| --------- | --- | ----- | ----- | --------------- |  
| 전 | 63.26ms | 83ms | 94.05ms | 787.81 |  
| 후 | 48.28ms | 66.92ms | 74.97ms | 1031.68 |  
